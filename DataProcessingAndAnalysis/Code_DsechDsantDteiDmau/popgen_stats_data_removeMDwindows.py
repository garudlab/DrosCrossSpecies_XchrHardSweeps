        ...
        if pos<=last_pos:
            ...
            while pos < window:
                ...
                if line == ['']:
                    ...
                    snp_list=line[1:]
                i+=1
            np_snp_window=np.array(snps_window)
            count_N = np.sum(np_snp_window == 'N') #cound number of N's in window
           
       
            if count_N > 0 :
                prop_N = count_N/(window_size*numStrains) #proportion of N's in window (prop of window lenght)
                #prop_N = count_N/float(np_snp_window.size) #proportion of N's in window prop of SNPs
                if prop_N >=0.5 : # if more than x% of sites in windows are missing
                    print("skipping,",str(prop_N*100),"% of missing data in window")
                    #restart variables
                    snps_window=[]
                    pi=0
                    continue #skip window
                   
            if len(snps_window)>=2:
                TajD=0 #calcTajimasD(np.asarray(snps_window))
                out_file.write(str(window)+'\t'+str(len(snps_window)/window_size)+"\t"+str(pi/window_size)+"\t"+str(len(snps_window)/(5.550497*window_size))+"\t"+str(singletons/window_size)+"\t"+str(0)+"\t" + str(TajD/window_size)+"\n") #change Waterson's theta constant
