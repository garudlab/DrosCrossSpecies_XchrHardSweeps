...
outFile=outFiles/Stats_NeutralFemaleBias${sexRatio}_${Ne}_Q${Q}_$chr_${id}.txt

for i in `seq 1 100`; do #1100

	#runSlim
	slim -d "file_path='${file}'" -d R=${rho_in}  -d N=${Ne} -d id=${id} -d Mu=${Mu}\
    -d Q=${Q}  -d "ChrType='${chr}'" -d burn_in=${burn_in} -d sexRatio=${sexRatio} Neutral_FemaleBias.slim
